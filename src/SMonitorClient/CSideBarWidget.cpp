...
void CSideBarWidget::addTab(const QString& text, const QChar& icon)
{
	...
	if(QString::fromLocal8Bit("软件更新") == text){
		sideTab->setShowTips(true);
		connect(this, &CSideBarWidget::TipsNumberChanged, sideTab, &CPushButton::setTipsNumber);
	}
	...
	m_mainLayout->insertWidget(count, sideTab);
}
